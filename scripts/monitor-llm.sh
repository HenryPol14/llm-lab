...
if ((PROXMOX_MODE)); then
  ...
  qm guest exec "$LLM_VMID" -- bash -lc 'uptime; df -h / /mnt/data 2>/dev/null || true; docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || true' || true
  qm guest exec "$MONITORING_VMID" -- bash -lc 'uptime; df -h / /mnt/data 2>/dev/null || true; docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || true' || true
fi

...
IP_CONFIG=$(qm guest exec "$MONITORING_VMID" -- ip -4 addr show 2>/dev/null | grep -E "inet " | tail -1)
echo "✓ IP конфигурация внутри VM:"
echo "  $IP_CONFIG"

if qm guest exec "$MONITORING_VMID" -- ip addr show | grep -q "${MONITORING_IP:-}"; then
...
qm guest exec "$MONITORING_VMID" -- lsblk 2>/dev/null | head -15 || echo "  (не удалось получить информацию)"

echo ""
echo "✓ Монтирование /mnt/monitoring-data:"
if qm guest exec "$MONITORING_VMID" -- test -d /mnt/monitoring-data 2>/dev/null; then
  echo "✓ Директория /mnt/monitoring-data существует"
  qm guest exec "$MONITORING_VMID" -- df -h /mnt/monitoring-data 2>/dev/null
else
...
qm guest exec "$MONITORING_VMID" -- grep "/mnt/monitoring-data" /etc/fstab 2>/dev/null || echo "  (запись не найдена)"

echo ""
echo "✓ Проверка пользователя ${GUEST_USER}:"
if qm guest exec "$MONITORING_VMID" -- id "$GUEST_USER" >/dev/null 2>&1; then
  echo "✓ Пользователь $GUEST_USER существует"
  OWNER=$(qm guest exec "$MONITORING_VMID" -- stat -c "%U:%G" /mnt/monitoring-data 2>/dev/null || echo "не удалось определить")
...
qm guest exec "$MONITORING_VMID" -- bash -lc 'command -v docker >/dev/null 2>&1 && echo "Docker installed" || echo "Docker not installed"'

CLOUD_INIT_STATUS=$(qm guest exec "$MONITORING_VMID" -- cloud-init status 2>/dev/null || true)

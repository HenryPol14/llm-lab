...
|--------------|------------|--------------|--------|
| 80 | Nginx proxy (10.10.10.70) | 80 | HTTP redirect → HTTPS |
| 443 | Nginx proxy (10.10.10.70) | 443 | HTTPS (Unified entry) |
...
### HTTPS Access (Unified Entry Point)

Все сервисы доступны через единый TLS endpoint `https://77.50.132.85`:

- **Root path (`/`)** — Open WebUI (port 3000)
- **`/ollama/`** — Ollama API (port 11434)
- **`/prometheus/`** — Prometheus (port 9090)
- **`/grafana/`** — Grafana (port 3000)
- **`/alertmanager/`** — Alertmanager (port 9093)

HTTP (80) автоматически перенаправляется на HTTPS (443).

...
DEBUG=1 bash -x scripts/01-install-proxmox-tools.sh
DEBUG=1 bash -x scripts/02-enable-iommu.sh
DEBUG=1 bash -x scripts/03-configure-network.sh
DEBUG=1 bash -x scripts/04-download-cloud-image.sh
DEBUG=1 bash -x scripts/05-create-cloudinit-template.sh
После 02-enable-iommu.sh, если IOMMU включался впервые, лучше перезагрузить Proxmox:
...
DEBUG=1 bash -x scripts/06-create-llm-vm.sh
DEBUG=1 bash -x scripts/07-create-monitoring-vm.sh
qm list
qm config 110
qm config 120
7. Деплой внутри VM

DEBUG=1 bash -x scripts/08-install-guest-runtime.sh 10.10.10.50
...
bash scripts/12-audit-network.sh
